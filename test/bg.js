...
bg['testing positive numbers from 1,000,000,000,000 to 999,999,999,999,999 , lowercase'] = function(done){
  assert.equal(check.bulgarianConversion(1000000000000,'lowercase'),'един билион');
  assert.equal(check.bulgarianConversion(1000000000001,'lowercase'),'един билион, и едно');
  assert.equal(check.bulgarianConversion(1001001002001,'lowercase'),'един билион, един милиард, един милион, две хиляди, и едно');
  assert.equal(check.bulgarianConversion(5002001024014,'lowercase'),'пет билиона, два милиарда, един милион, двадесет и четири хиляди, и четиринадесет');
  assert.equal(check.bulgarianConversion(18012003005300,'lowercase'),'осемнадесет билиона, дванадесет милиарда, три милиона, пет хиляди, и триста');
  assert.equal(check.bulgarianConversion(78065002018024,'lowercase'),'седемдесет и осем билиона, шестдесет и пет милиарда, два милиона, осемнадесет хиляди, двадесет и четири');
  assert.equal(check.bulgarianConversion(836905113950060,'lowercase'),'осемстотин тридесет и шест билиона, деветстотин и пет милиарда, сто и тринадесет милиона, деветстотин и петдесет хиляди, и шестдесет');
  assert.equal(check.bulgarianConversion(200100072950060,'lowercase'),'двеста билиона, сто милиарда, седемдесет и два милиона, деветстотин и петдесет хиляди, и шестдесет');
  assert.equal(check.bulgarianConversion(905102803107002,'lowercase'),'деветстотин и пет билиона, сто и два милиарда, осемстотин и три милиона, сто и седем хиляди, и две');
  assert.equal(check.bulgarianConversion(2000316120999,'lowercase'),'два билиона, триста и шестнадесет милиона, сто и двадесет хиляди, деветстотин деветдесет и девет');
  assert.equal(check.bulgarianConversion(900000000000001,'lowercase'),'деветстотин билиона, и едно');
  assert.equal(check.bulgarianConversion(111999000999030,'lowercase'),'сто и единадесет билиона, деветстотин деветдесет и девет милиарда, деветстотин деветдесет и девет хиляди, и тридесет');
  done();
};
